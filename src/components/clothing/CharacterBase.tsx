...
export function CharacterBase({ outfit, selectedItem, onItemClick }: CharacterBaseProps) {
  const meshRef = useRef<Mesh>(null);

  useFrame((state) => {
    if (meshRef.current) {
      meshRef.current.scale.y = 1 + Math.sin(state.clock.elapsedTime * 2) * 0.01;
    }
  });

  const getShirtColor = () => {
    switch (outfit) {
      case "casual": return "#ffffff";
      case "street": return "#1a1a2e";
      case "formal": return "#636e72";
      case "sporty": return "#fdcb6e";
  ...
  const getPantsColor = () => {
    switch (outfit) {
      case "casual": return "#0984e3";
      case "street": return "#000000";
      case "formal": return "#2d3436";
      case "sporty": return "#e17055";
  ...
  const getShoeColor = () => {
    switch (outfit) {
      case "casual": return "#ffffff";
      case "street": return "#ff0066";
      case "formal": return "#2d3436";
      case "sporty": return "#00b894";
      default: return "#ffffff";
    }
  };
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0.3, 0]} castShadow onClick={() => onItemClick("shirt")}>
      ...
      <mesh position={[-0.2, -0.8, 0]} castShadow onClick={() => onItemClick("pants")}>
      ...
      {selectedItem && (
        <Html position={[0, 2, 0]} center>
          <div style={{
            ...
            {selectedItem.charAt(0).toUpperCase() + selectedItem.slice(1)} Selected
          </div>
        </Html>
      )}
    </group>
